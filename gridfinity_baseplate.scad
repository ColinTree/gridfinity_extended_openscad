...
// X轴外部尺寸 / X outer dimension. grid units (multiples of 42mm) or mm.
outer_Width = [0, 0]; //0.1
// Y轴外部尺寸 / Y outer dimension. grid units (multiples of 42mm) or mm.
outer_Depth = [0, 0]; //0.1
// Z轴外部尺寸 / z outer dimension. mm.
outer_Height = 0; //0.1
...
/* [打印床设置 / Printer bed options] */
build_plate_enabled = "disabled";//[disabled, enabled, unique]
//均匀分布板块 / spread out the plates, use if last row is small.
average_plate_sizes = false;
//分割板块 / Will split the plate in to the
...
//磁铁Z轴偏移 / raises the magnet, and creates a floor (for gluing)
Magnet_Z_Offset = 0;  // .1
//磁铁顶盖 / raises the magnet, and creates a ceiling to capture the magnet
Magnet_Top_Cover = 0;  // .1
// [Magnet Release Options]
// 磁铁取出方式 / Method to help remove magnets: "none", "slot" (side pry), "hole" (poke from behind)
