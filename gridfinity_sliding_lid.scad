...
/* [Sliding Text / 滑盖文字] */
// 添加文字到滑盖顶面 / Add text to the sliding lid top
sliding_lid_text_enabled = false;
// 盖面文字内容 / Text to display on the lid
sliding_lid_text = "Gridfinity";
// 文字字体大小（0=自动） / Font size for the lid text (0 = auto-size)
sliding_lid_text_size = 0; // 0.1
// 文字雕刻深度 / Depth of text engraving in mm
...
// 杯沿顶部凹槽高度 / Create a relief cut in the lip
...
// 启用取物圆角的墙面 / wall to enable on, front, back, left, right. 0: disabled; 1: enabled;
...
// 标签尺寸（高度、宽度、厚度、样式）宽度默认=高度，厚度默认1.4，样式{0,1,2} / Tab size, height, width, thickness, style. width default is height, thickness default is 1.4, style {0,1,2}.
