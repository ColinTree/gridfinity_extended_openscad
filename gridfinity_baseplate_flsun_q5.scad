...
// FLSUN Q5 螺丝孔间距（mm）/ distance between existing screw holes on FLSUN q5.
ear_hole_x = 182.5;
// 螺丝孔距前面板距离（mm）/ distance of screw hole from the front panel.
ear_hole_y = 7;
// 底板厚度（mm）/ height from above.
cube_z = 4.4;
// M4螺栓直径（mm）/ diameter needed for an M4 bolt.
M4_d = 4.2;

// 壁嵌入厚度（mm），用于嵌入角落 / Extend and imbed in to wall to fit around corner
wallThickness = 0.2;
