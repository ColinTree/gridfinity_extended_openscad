...
// 孔配合间隙 / Enlarge the holes by this amount for clearance
itemholder_hole_clearance = 0.25;
// 孔深度 / Depth of hole, Overrides the known item depth. Limited by floor height.
itemholder_hole_depth = 0; //0.1
// 孔顶部倒角 / 45 deg chamfer added to the top of the hole (mm)
itemholder_hole_chamfer = 1; //0.5

/* [物品固定器 - 样品 / Item Holder - Sample Item] */
//样品模式 / Render just a sample of the item hole, to be used as a test print
itemholder_enable_sample = false;
//样品壁厚 / Wall thickness of the sample print
itemholder_sample_wall_thickness = 3;

/* [物品固定器 - 多卡槽 / Item Holder - Multi Card] */
// 多卡卡种 / cards to use when multi card is selected I.E. sd;USBA;microsd
itemholder_multi_cards = "sd,usba,microsd";
// 多卡紧凑排列 / Force nesting of multi cards, This has an issue where the last item could be cropped.
itemholder_multi_card_compact = 0.7; // [0:0.1:1]

/* [物品固定器 - 自定义 / Item Holder - Custom Item] */
// 网格样式 / Should the grid be square or hex
itemholder_hole_base_shape = "round"; //["round","square","halfround","multicard","custom":custom shape - beta feature]
// 孔边数 / The number of sides for a round hole
itemholder_hole_sides = 4;
// 孔直径 / Diameter of, round hole, or corners for square hole
itemholder_hole_diameter = 5; //0.1
// 孔底圆角 / Radius of the bottom of the custom shape
itemholder_hole_bottom_radius = 0;
// 孔尺寸 / The size the hole
itemholder_hole_size = [20, 25]; //0.1
itemholder_hole_rotation = 0;

/* [物品固定器 - 布局 / Item Holder - Item Layout] */
// 网格样式 / Should the grid be square or hex
itemholder_grid_style = "auto"; //["square","hex","auto"]
//孔间距 / Spacing around the holes
itemholder_hole_spacing = 2; //0.1
// Y轴 / Number of holes in the x and y dimension, 0 is dynamic
itemholder_hole_gridx = 0; //1
// Y轴 / Number of holes in the y dimension, 0 is dynamic, y.5, is only valid for hex.
itemholder_hole_gridy = 0; //0.5
//自动高度 / Auto set the bin height based on the hole size.
itemholder_auto_bin_height = "enabled"; //["enabled","enabled_full","disabled"]
// 孔边数 / The number of sides for a round hole
itemholder_compartments = [1,1]; //[1:10]
// 分区间距 / Spacing around the compartments
itemholder_compartment_spacing = 3; //0.1
// 居中对齐 / Center the holes within the compartments
