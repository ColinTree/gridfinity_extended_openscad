...
/* [Subdivisions / 分隔] */
// 分隔墙厚度 [底, 顶] / Wall thickness [bottom, top]
chamber_wall_thickness = [1.2, 1.2]; //0.1
// 分隔墙高度减量 / Reduce the wall height by this amount
chamber_wall_headroom = 0;//0.1
// 分隔墙顶部圆角半径 / Radius of the top of the chamber wall, -ve is ratio of top wall thickenss. (disabled for bent walls)
...
// 嵌入式磁铁侧面开口 / Use with captive magnet for a 'refinded style' magnet
magnet_side_access = "disabled";//[disabled,left:"↰ left",right:"⬑ right"]
// 嵌入式磁铁高度偏移 / raise the magnet void inside the part for print-in-magnets
magnet_captive_height = 0; // .1
// 磁铁孔波浪纹深度 / add a wavy pattern to the magnet hole
...
// 仅在角落添加附件（加快打印） / Only add attachments (magnets and screw) to box corners (prints faster).
...
// 平底（移除内部网格） / Removes the internal grid from base the shape
flat_base = "off"; // [off, gridfinity:gridfinity stackable, rounded]
// 垂直间隔片（移除底面） / Remove floor to create a vertical spacer
spacer = false;
// 最小可打印底座尺寸 / Pads smaller than this will not be rendered as it interferes with the baseplate. Ensure appropriate support is added in slicer.
...
// 平底圆角便于打印的倒角 / Add chamfer to the rounded bottom corner to make easier to print. -1 add auto 45deg.
flat_base_rounded_easyPrint = -1;
// 网格X对齐 / grid position x
align_grid_x = "near";//[near, far]
// 网格Y对齐 / grid position y
...
/* [Floor Pattern / 底面镂空] */
// 启用底面镂空 / enable Grid floor patter
floorpattern_enabled=false;
// 底面镂空样式 / Style of the pattern
...
text_1 = false;
// 文字字体大小（mm，0=自动） / Font Size of text, in mm (0 will auto size)
text_size = 0; // 0.1
