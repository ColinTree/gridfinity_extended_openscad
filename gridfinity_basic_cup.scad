...
// 唇边顶部缺口高度 / Create a relief in the lip
lip_top_relief_height = -1; // 0.1
// 唇边两端保留比例 / how much of the lip to retain on each end
lip_top_relief_width = -1; // 0.1
// 添加唇边防滑凸点 / add a notch to the lip to prevent sliding.
lip_top_notches  = true;
// 启用唇边卡扣连接 / enable lip clip for connection cups
lip_clip_position = "disabled"; //[disabled, intersection, center_wall, both]
//非42mm倍数时允许堆叠 / allow stacking when bin is not multiples of 42
...
// 磁铁空腔抬高（打印时嵌入磁铁） / raise the magnet void inside the part for print-in-magnets
magnet_captive_height = 0; // .1
// 磁铁孔波浪纹（增加过盈配合） / add a wavy pattern to the magnet hole
...
// 网格对齐方向 X / grid position x
align_grid_x = "near";//[near, far]
// 网格对齐方向 Y / grid position y
...
// 启用内部分隔板标签 / Enable labels on internal divider walls
...
// 收纳盒间隙（mm） / clearance around the bin, will reduce the bin by this amount in mm.
clearance = [0.5, 0.5, 0];
