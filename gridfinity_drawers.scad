...
/* [Chest] */
//抽屉内宽（Gridfinity单位） / Inner width of drawer
drawer_inner_width = 4;
//抽屉内深（Gridfinity单位） / Inner depth of drawer
drawer_inner_depth = 3;
//抽屉内高（Gridfinity单位） / Inner height of drawer
drawer_inner_height = 4;
//抽屉数量 / Number of drawers
drawer_count = 3;
drawer_enable_custom_sizes = false;
//抽屉内高（Gridfinity单位） / Inner height of drawer. Edit in script for more than 4 items.
drawer_custom_sizes = [1,2,3,4];
//为托盒添加抽屉内间隙 / Add clearance inside the drawers. Width, depth and height. Default is 0.25
drawer_clearance = [0.25,0.25,0.25];
//为抽屉添加柜体内间隙 / Add clearance inside the chest. width, depth and height. Default is 0.25
chest_clearance = [0.25,0.25,0.25];
//壁厚 / 柜体壁厚 / Wall thickness of the chest.
chest_wall_thickness = 2; // 0.1
//抽屉滑轨厚度（mm） / Thickness of drawer slides. 0 is uses wall thickness.
chest_drawer_slide_thickness = 0;
//抽屉滑轨宽度（mm） / Width of drawer slides. 0 is full chest width.
chest_drawer_slide_width = 10;

/* [Drawer] */
// 手柄尺寸（宽、深、高、圆角） / Handle size width, depth, height, and radius. Height, less than 0 drawerHeight/abs(height). radius, -1 = depth/2.
...
/* [Chest Wall Pattern] */
// 墙面图案边框宽度 / wall pattern border width. -1 defaults to chest_wall_thickness. less than 0 chest_wall_thickness/abs(wallpattern_border_width)
