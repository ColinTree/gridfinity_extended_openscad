...
/* [抽屉柜 / Chest] */
//抽屉内部宽度（GF单位） / Inner width of drawer in Gridfinity units
drawer_inner_width = 4;
//抽屉内部深度（GF单位） / Inner depth of drawer in Gridfinity units
drawer_inner_depth = 3;
//抽屉内部高度（GF单位） / Inner height of drawer in Gridfinity units
drawer_inner_height = 4;
//抽屉数量 / Number of drawers
drawer_count = 3;
drawer_enable_custom_sizes = false;
//各抽屉高度（GF单位，超过4个需编辑脚本） / Inner height of drawer in Gridfinity units. Edit in script for more than 4 items.
drawer_custom_sizes = [1,2,3,4];
//收纳盒与抽屉内壁间隙（宽/深/高） / Add clearance inside the drawers for the bins.
drawer_clearance = [0.25,0.25,0.25];
//抽屉与抽屉柜间隙（宽/深/高） / Add clearance inside the chest for the drawer.
chest_clearance = [0.25,0.25,0.25];
//抽屉柜壁厚 / Wall thickness of the chest.
chest_wall_thickness = 2; // 0.1
//抽屉滑轨厚度（mm，0=壁厚） / Thickness of drawer slides in mm. 0 is uses wall thickness.
chest_drawer_slide_thickness = 0;
//抽屉滑轨宽度（mm，0=全宽） / Width of drawer slies in mm. 0 is full chest width.
chest_drawer_slide_width = 10;

/* [抽屉 / Drawer] */
// 把手尺寸（宽/深/高/圆角） / Handle size width, depth, height, and radius.
...
//仅在角落添加磁铁/螺丝（加快打印） / Only add attachments (magnets and screw) to chest corners
...
/* [柜壁图案 / Chest Wall Pattern] */
// 图案边框宽度 / wall pattern border width.
