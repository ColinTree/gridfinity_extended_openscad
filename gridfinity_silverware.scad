...
// 杯体高度（不含杯沿） / Height upper surface excluding perimeter lip. . grid units (multiples of 7mm) or mm.
height = [0, 42];

/* [Other parameters / 其他参数] */
// 分隔墙厚度 [底, 顶] / Wall thickness [bottom, top], top -ve is ratio of bottom wall thickness
...
text_1 = false;
// 文字字体大小 / Font Size of text, in mm (0 will auto size)
text_size = 0; // 0.1
...
// X轴宽度（所有餐具合计，Gridfinity单位）/ X, width of combination of all silverware, in gridfinity units
width = ceil((totwidth(silver_defs) + silverware_wall_clearance)/42);
// Y轴深度（所有餐具最大深度，Gridfinity单位）/ Y, max depth of all silverware, in gridfinity units
